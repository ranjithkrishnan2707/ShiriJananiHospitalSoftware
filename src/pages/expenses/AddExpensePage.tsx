...
const AddExpensePage: React.FC = () => {
  ...
  const paramDept = searchParams.get('dept') as DepartmentType | null;

  const initialDept: DepartmentType = (paramDept && ['Medical', 'Lab', 'Scan', 'OPD', 'IPD', 'Admin', 'Maintenance', 'Other'].includes(paramDept))
    ? paramDept
    : 'Medical';

  // Determine back navigation and label based on origin
  const getBackRoute = () => {
    if (fromSource === 'doctor') return '/doctor';
    if (fromSource === 'lab') return '/lab';
    if (fromSource === 'medical') return '/medical';
    return '/admin/expenses';
  };

  const getBackLabel = () => {
    if (fromSource === 'doctor') return 'Back to Doctor Consultation';
    if (fromSource === 'lab') return 'Back to Lab Dashboard';
    if (fromSource === 'medical') return 'Back to Medical / Pharmacy';
    return 'Back to All Expenses';
  };
  ...
  const [department, setDepartment] = useState<DepartmentType>(initialDept);
  ...
  return (
    <div className="add-expense-page-container page-transition">
      <div className="add-expense-page-header">
        <h3>
          <PlusCircle size={22} /> Add New Expense Record
        </h3>
      ...
      <form onSubmit={handleSubmit} className="add-expense-page-body">
        {errorMsg && (
          <div style={{
        ...
        <div className="form-section-card">
          ...
          <div className="form-row-grid-3">
            <div className="form-field-group">
              <label>Expense ID (Auto Generated)</label>
              <input 
                type="text" 
                className="form-control" 
                value={generatedId} 
                disabled 
                style={{ backgroundColor: '#ffffff', fontWeight: 700, color: '#be185d', fontSize: '15px' }}
              />
            </div>
        ...
        <div className="form-section-card">
          ...
          <div className="form-row-grid-2">
            <div className="form-field-group">
              ...
              <input 
                type="text" 
                className="form-control" 
                placeholder="Select or type vendor / supplier name"
                list="page-vendors-list"
                value={vendor}
                onChange={(e) => setVendor(e.target.value)}
                required
              />
              <datalist id="page-vendors-list">
                {vendors.map(v => (
          ...
          {/* Row 4: Amount, Payment Mode & Payment Status */}
          <div className="form-row-grid-3">
            <div className="form-field-group">
        ...
        <div className="form-section-card">
          <div className="form-section-title">
            3. Description, Attachments & Notes
          </div>
        ...
        {/* System Audit Information Bar */}
        <div className="system-info-bar">
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <User size={15} color="#be185d" /> Entered By: <strong>Dr. Admin</strong>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <Clock size={15} color="#be185d" /> Created Date & Time: <strong>Automatic ({expenseDate} {expenseTime})</strong>
          </div>
        </div>

        {/* Action Buttons */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '14px', marginTop: '8px' }}>
          <button 
            type="button" 
            className="btn-reset-filters"
            onClick={() => navigate(getBackRoute())}
          ...
          >
            <Save size={18} /> Save Expense Record
          </button>
        </div>
      </form>
    </div>

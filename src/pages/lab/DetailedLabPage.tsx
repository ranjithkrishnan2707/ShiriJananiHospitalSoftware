...
const DetailedLabPage: React.FC = () => {
  ...
  const initialTab = (searchParams.get('tab') as any) || (location.state as any)?.tab || 'patient-entry';
  const { patients, labRequests, markLabComplete, doctors, openDoctorListModal } = useHospital();

  // Navigation Sub-Bar & Active Views State
  const [activeTab, setActiveTab] = useState<'patient-entry' | 'test-entry' | 'test-result' | 'print-result' | 'bill-print' | 'test-group' | 'test-wise-report'>(initialTab);
      ...
      <header className="lab-navbar">
        <nav className="lab-nav-links">
          ...
          <button 
            ...
            style={{ background: 'linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%)', color: 'white', fontWeight: 700 }}
          >
